...
describe("Legal Pages Seed", () => {
  ...
  it("should seed Terms of Service content", async () => {
...
`;
    ...
    expect(result?.value).toContain("Aceitação dos Termos");
  });

  it("should seed Privacy Policy content", async () => {
    const caller = appRouter.createCaller(adminContext);
...
`;
    ...
    expect(result?.value).toContain("Informações que Coletamos");
  });

  it("should retrieve seeded Terms of Service", async () => {
    const caller = appRouter.createCaller(adminContext);
    
    const result = await caller.siteSettings.get({ key: "terms_of_service" });
    
    expect(result).toBeDefined();
    expect(result?.value).toContain("Aceitação dos Termos");
  });

  it("should retrieve seeded Privacy Policy", async () => {
    const caller = appRouter.createCaller(adminContext);
    
    const result = await caller.siteSettings.get({ key: "privacy_policy" });
